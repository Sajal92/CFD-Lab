...
void write_vtkFile(const char *szProblem,
	...
	{
		char szBuff[80];
		sprintf( szBuff, "Failed sdfto open %s", szFileName );
		ERROR( szBuff );
		return;
	}
	...
	for(k = 0; k < kmax+1; k++) {
		for(j = 0; j < jmax+1; j++) {
			for(i = 0; i < imax+1; i++) {
				uVel = ValidValue((U[i][j][k]+ U[i][j+1][k] + U[i][j][k+1] + U[i][j+1][k+1]) * 0.25);
				vVel = ValidValue((V[i][j][k]+ V[i+1][j][k]+  V[i][j][k+1] + V[i+1][j][k+1]) * 0.25);
				wVel = ValidValue((W[i][j][k]+ W[i+1][j][k] + W[i][j+1][k] + W[i+1][j+1][k]) * 0.25);
				fprintf(fp, "%f %f %f\n",uVel,vVel,wVel);
			}
		}
	}
