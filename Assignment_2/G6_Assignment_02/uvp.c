...
void calculate_uv(double dt,double dx,double dy,int imax, int jmax,double**U, double**V,double**F,double**G,double **P,int **flag){
	for (int i = 0; i< imax-1;i++){
		for (int j = 0; j<jmax;j++)
		{
			if(((flag[i][j]&(1<<0))&flag[i+1][j]) || ( (flag[i+1][j] & (1<<3)) && (flag[i][j]&(1<<0))))//update the U component of velocity
	...
	for (int i = 0; i< imax;i++){
		for (int j = 0; j<jmax-1;j++){
			if((flag[i][j]&(1<<0))&flag[i][j+1])//update the V component of velocity
	...
	for (int i = 0; i< imax;i++){
		for (int j=0; j<jmax;j++){
			if(flag[i][j]&((1<<0)|(1<<3)|(1<<4))){
		  		...
		  		TXX = (1/(Re*Pr))*(dt2_dx2+dt2_dy2) - dut_dx - dvt_dy;

				temp[i][j] = temp1[i][j]+ (dt*TXX);
			}
		}
	}
...
void reset_obstacles(double **U, double **V, double **P, double **T, int **flag, int imax, int jmax,int include_temp){
	for (int i = 0; i< imax;i++){
		for (int j=0;j<jmax; j++){
			if(flag[i][j]&( (1<<1)|(1<<2)) ){
					U[i][j] = 0;
